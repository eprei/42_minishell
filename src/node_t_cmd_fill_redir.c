...
/*   Updated: 2022/07/15 15:24:21 by epresa-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fill_pipe(t_var *v, t_prompt *prompt, int *i, int *n_pipe)
{
	...
	if (v->s_split[*i][0] == '|')
	{
		...
		{
			printf("\nn_pipe = %d\n", *n_pipe);
			printf("asigning pipe at s_split[%d]\tpipe prev outfile = %d\tpipe curr infile = %d\n\n", *i, prompt->pipes[*n_pipe][1], prompt->pipes[*n_pipe][0]);
		...
		{
			printf("\nn_pipe = %d\n", *n_pipe);
			printf("closing pipe at s_split[%d]\t prev outfile = %d\tpipe curr infile = %d\n\n", *i, prompt->pipes[*n_pipe][1], prompt->pipes[*n_pipe][0]);
